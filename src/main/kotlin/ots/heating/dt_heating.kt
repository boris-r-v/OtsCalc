 ...
 */
interface DT_temp_storage {
    /**
     * Сохранить новые данные
     */
    fun append( current: DT_current, temp: DT_temp )
}
/**
...
class DT(var temp: DT_temp, val type: DT_type, var ht: DT_heat_transfer){
    ...
    {
        val f1 = type.coil_R*(1+0.004*(coil_temp-293))*current_.pow(2)
     ...
     * @param writer - наследник интерфейса DT_temp_storage, который умеет сохранять рассчитанные данные
     */
    fun calc( tok: DT_current, writer: DT_temp_storage )
    {
    ...
    fun calc( items: Array<DT_current>, writer: DT_temp_storage){
        items.forEach { calc(it, writer) }
    }
    fun calc(items: Collection<DT_current>, writer: DT_temp_storage){
        items.forEach { calc(it, writer) }
    }
